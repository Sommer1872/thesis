...
def process_daily_statistics(results: List[Dict]) -> pd.DataFrame:
    ...
    for date_stats in tqdm(results):
        ...
        for orderbook_no, this_orderbook_stats in all_statistics.items():
            ...
            metadata.loc[orderbook_no, "time_weighted_quoted_spread_bps"] = best_bid_ask_stats["time_weighted_relative_quoted_spread_bps"]
            # best_depth
            best_depth_stats = this_orderbook_stats["best_depth_stats"]
            metadata.loc[orderbook_no, "time_weighted_average_depth"] = best_depth_stats["time_weighted_average_depth"]

            # realized vola
            realized_vola_stats = this_orderbook_stats.get("realized_vola_stats", dict())
            if realized_vola_stats:
                ...
                metadata.loc[orderbook_no, "noise_var_TSRV"] = realized_vola_stats["noise_var_TSRV"]
                metadata.loc[orderbook_no, "signal_to_noise"] = realized_vola_stats["signal_to_noise"]
