...
def calculate_orderbook_stats(this_day_imi_data):
    ...
    for orderbook_no in metadata.index:
        ...
        stats = pd.DataFrame.from_dict(
            this_day_imi_data.orderbook_stats[orderbook_no], orient="index"
        )
        stats["quoted_spread"] = stats["best_ask"] - stats["best_bid"]
        stats["mid"] = (stats["best_ask"] + stats["best_bid"]) * 0.5
        stats["relative_quoted_spread_bps"] = (
            100 * stats["quoted_spread"] / stats["mid"]
        )
        stats[["quoted_spread", "mid"]] /= price_decimals
        stats[["best_ask", "best_bid"]] = (
            stats[["best_ask", "best_bid"]] / price_decimals
        )
        ...
        transactions = pd.DataFrame.from_dict(
            this_day_imi_data.transactions[orderbook_no], orient="index"
        )
        transactions["mid"] = (
        ...
        transactions["relative_effective_spread_bps"] = (
            100
            * 2
            * np.abs(transactions["price"] - transactions["mid"])
            / transactions["mid"]
        )
        ...
        tick_sizes = pd.DataFrame.from_dict(
            this_day_imi_data.price_tick_sizes[
                int(metadata.loc[orderbook_no].price_tick_table_id)
            ],
            orient="index",
        )
        ...
        transactions["spread_leeway"] = (
            transactions["effective_spread"] / transactions["tick_size"]
        )
