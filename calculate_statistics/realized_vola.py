...
def calculate_realized_vola_stats(transactions: pd.DataFrame):
    ...
    transactions.set_index(pd.Timestamp("2019-01-01") + pd.to_timedelta(transactions["timestamp"], unit="us"), inplace=True)
    ...
    offsets = [frac * multiple for multiple in range(int(low_sampling_frequency / high_sampling_frequency))]
